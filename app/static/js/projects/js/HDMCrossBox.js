/* eslint-disable */
var HDMCrossBox = /** @class */ (function () {
    function HDMCrossBox(sgworld) {
        this.mSGWorld = sgworld;
        this.isStart = false;
        this.prelc = -1;
    }
    HDMCrossBox.prototype.Start = function () {
        if (!mileageReady) {
            alert('! mileageReady');
            return;
        }
    ...
    // }
    HDMCrossBox.prototype.MSGWorld_OnLButtonDown = function (Flags, X, Y) {
        if (!this.isStart) {
            console.log(X, Y);
            ShowHDMCross(X, Y);
    ...
    HDMCrossBox.prototype.MSGWorld_OnFrame = function () {
        var pos = this.mSGWorld.Window.GetMouseInfo();
